...
class TakkTile:
	...
	def getData(self):
		...
		for cell in self.alive:
			...
			# arr.data.insert(cell, round(Pcomp[cell], 4))
			# pub.publish(arr)
			# arr.data[:]=[]
			# rate.sleep()
		return dict(zip(self.alive, zip(Pcomp, Tcomp)))
...
if __name__ == "__main__":
	...
	rospy.init_node('tactile_glove', anonymous=True)
	pub = rospy.Publisher('glove_data', Float32MultiArray, queue_size =100)
