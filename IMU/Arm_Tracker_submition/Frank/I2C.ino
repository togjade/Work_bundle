...
void Read_Gyro(uint8_t i)
{
  ...
  gyro_x = SENSOR_SIGN[0] * (AN[0] - AN_OFFSET[i][0]);
  gyro_y = SENSOR_SIGN[1] * (AN[1] - AN_OFFSET[i][1]);
  gyro_z = SENSOR_SIGN[2] * (AN[2] - AN_OFFSET[i][2]);
...
void Accel_Init()
{
  ...
  gyro_acc.writeReg(LSM6::CTRL1_XL, 0x4C); // 104 Hz, 8 g full scale
#else
  compass.init();
...
void Read_Accel(uint8_t i)
{
  ...
  AN[5] = compass.a.z >> 4;
#endif
  accel_x = SENSOR_SIGN[3] * (AN[3] - AN_OFFSET[i][3]);
  accel_y = SENSOR_SIGN[4] * (AN[4] - AN_OFFSET[i][4]);
  accel_z = SENSOR_SIGN[5] * (AN[5] - AN_OFFSET[i][5]);
